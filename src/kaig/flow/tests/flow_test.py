...
def test_flow():
    ...
    def chunk_flow(record: Record, flow: Flow):  # pyright: ignore[reportUnusedFunction]
        # create chunk
        ...
        assert flow.stamp == "chunked"

    @exe.flow(table="chunk", stamp="meta", priority=2)
    def metadata_flow(record: Record, flow: Flow):  # pyright: ignore[reportUnusedFunction, reportUnusedParameter]
        _ = db.sync_conn.query(
